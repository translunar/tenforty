"""Synthetic 2022 CA-resident emit test.

Proves the whole 2022 CA path end-to-end: synthetic CA-resident scenario
with year=2022 → run_full_california_return → the 2022 CA packet
(f540 + sch_ca + sch_d_540) emits with non-trivial size onto the 2022
templates.

Ports tests/test_ca_2023_emit.py with year overridden to 2022 via
dataclasses.replace — the final Step-7 check that 2022's promotion to the
...
class CA2022EmitTests(unittest.TestCase):
    def test_2022_ca_packet_emits(self):
        base = _make_ca_v1_smoke_scenario()
        scenario = dataclasses.replace(
            base,
            config=dataclasses.replace(base.config, year=2022),
            ca540=None,
        )
        ...
        # The emitted files must be the 2022 templates.
        self.assertTrue(str(ca_pdfs["f540"]).endswith("f540_2022.pdf"))
