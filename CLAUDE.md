...
6. **Adding a params field is a three-part change.** Two shape gates fire on any new `FederalParams` field: the battery coverage map (`tests/test_battery_param_coverage.py`) and the per-year attestation modules (`tests/params_attestations/`). So every param addition requires: (a) the param itself; (b) an attestation entry INDEPENDENTLY TRANSCRIBED from statute/official publications by a separate agent who has NOT seen the param values, the plan, or any message stating them — the params author must never write their own attestation, or the cross-check becomes a tautology; the transcription brief states ONLY the citation and the question — never values and never structural characterizations of the answer ("one status differs", "there's a phase-out") — and a reviewer checks the brief for leakage before dispatch, since the brief's author is contaminated; (c) a coverage-map decision, EXERCISED preferred (add a battery scenario that drives the param's binding case) — WAIVED needs a written justification. A transcription-vs-params disagreement routes to team-lead unreconciled; that's the mechanism working.
