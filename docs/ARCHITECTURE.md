...
- **`total_tax` means different things on the two 1040 paths — INTENTIONAL, do not "unify"**: the native spine's `total_tax` is line-16 tax ONLY (Schedule 2 additions — excess-APTC repayment, Form 8959 — join `overpaid`, not `total_tax`; 1040-X line 6 composes from this line-16 base). The workbook's production `total_tax` OUTPUT points at the `Tax` named range, which is `SUM(Tax_SubTotal, Schedule2_Tax)` — Schedule-2-INCLUSIVE — because its only production consumer is the out-of-native-spine **workbook-fallback** path (MFJ/EIC filers), which feeds Form 4868 balance-due, and a 4868 balance is computed against FULL liability including Schedule 2. Pointing the fallback OUTPUT at the pure line-16 range `Tax_SubTotal` would (a) understate a fallback filer's 4868 balance due and (b) surface `None` where `Tax`'s SUM currently numeric-coerces a blank `Tax_SubTotal` (the AL96-style formula can evaluate to `""`). The line-16-only quantity the native-vs-workbook parity test needs is therefore exposed as a SEPARATE, parity-only OUTPUT key `total_tax_line16` (← `Tax_SubTotal`); the parity loop maps native `total_tax` → oracle `total_tax_line16` explicitly. Sites: `mappings/f1040.py` OUTPUTS, `tests/test_f1040_spine_oracle.py` `_PARITY_ORACLE_KEY`, `tests/test_f1040_mapping.py` core-output pins. (Surfaced 2026-07-14 when the MFJ 4868 e2e went `None` after a naive remap.)
