...
├── scripts/
│   └── verify_no_personal_data.py  # Repo scan for personal data leaks
├── tests/
...
### Task 2: Personal Data Leak Verification

**Files:**
- Create: `scripts/verify_no_personal_data.py`
- Create: `tests/test_no_personal_data.py`
- Create: `scripts/personal_data_config.yaml` (gitignored)
- Create: `.git/hooks/pre-commit`

This task goes immediately after scaffolding so that every subsequent commit is gated by the personal data check. The script scans for: an allowlist of known synthetic identifiers, a denylist of real-world identifiers, and heuristics for suspicious patterns.
...
- [ ] **Step 4: Create the gitignored config file with real identifiers**
...
- [ ] **Step 6: Run the script directly to see output**
...
- [ ] **Step 7: Install as a git pre-commit hook**
...
Only commit the script and test. Do NOT commit the config file. (The hook lives in `.git/hooks/` which is not tracked by git. The .gitignore already covers `scripts/personal_data_config.yaml` from Task 1.)
...
### Task 5: F1040 Mapping (Core Federal Inputs/Outputs)
...
### Task 8: Scenario-to-Inputs Flattener
...
### Task 12: Verify Full Pipeline With 2024 Known-Good Data
...
| 2 | Personal data leak verification + pre-commit hook | 1 test + script |
| 3 | Data models | 6 tests |
| 4 | FormMapping registry | 7 tests |
| 5 | F1040 mapping | 11 tests |
| 6 | SpreadsheetEngine | 1 integration test |
| 7 | Scenario YAML loader | 5 tests |
| 8 | Scenario-to-inputs flattener | 6 tests |
| 9 | End-to-end integration | 1 test |
| 10 | Return orchestrator | 1 test |
| 11 | PDF filler | 2 tests |
| 12 | Realistic verification | 1 test |
